...
- `GET /health` → `{ "ok": true }`
- `GET /resolve/:name` → `{ address, records, owner, resolver, expires_at, expiresAt, fqdn, namehash }`
  - `records` map keys are hex-encoded record keys.
  - `address` resolves to the `addr` record when present.
  - `expires_at`/`expiresAt` are ISO timestamps derived from the on-chain `expires_at` seconds value.
